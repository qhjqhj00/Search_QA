...
def test(model, args, eval_examples, eval_features, device):
    ...
    for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        batch_start_logits_list = []
        batch_end_logits_list = []
        with torch.no_grad():
            for idx in models:
                tmp_start, tmp_end = models[idx]['model'](input_ids, segment_ids, input_mask)
                batch_start_logits_list.append(models[idx]['weight'] * tmp_start)
                batch_end_logits_list.append(models[idx]['weight'] * tmp_end)

        for i, example_index in enumerate(example_indices):
            start_list = []
            end_list = []
            for st in batch_start_logits_list:
                start_list.append(st[i].detach().cpu())
            for end in batch_end_logits_list:
                end_list.append(end[i].detach().cpu())
            
            start_logits = (sum(start_list)).tolist()
            end_logits = (sum(end_list)).tolist()
...
if __name__ == '__main__':
    ...
    model_path_list = glob(args.agg_model_dir + '*.pth')
